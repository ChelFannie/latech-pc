<template>
  <div class="system">
      <el-tabs class="wrap" v-model="activeName" type="card" @tab-click="handleClick">
        <el-tab-pane v-for="(item, index) in routerData" :key="index" :label="item.label" :name="item.name"></el-tab-pane>
      </el-tabs>
      <router-view></router-view>
  </div>
</template>
<script>
export default {
  data () {
    return {
      activeName: '0',
      routerData: [
        {label: '设备店铺', name: '0', pathName: {name: '设备店铺'}},
        {label: '开奖记录', name: '1', pathName: {name: '开奖记录'}},
        {label: '业务类别', name: '2', pathName: {name: '业务类别'}}
        // {label: '组织结构', name: '2', pathName: {name: '组织结构'}},
        // {label: '人员权限', name: '3', pathName: {name: '人员权限'}},
        // {label: '职务', name: '4', pathName: {name: '职务'}},
      ]
    }
  },
  mounted () {
    this.$router.currentRoute.redirectedFrom ? localStorage.setItem('activeName', '0') : (this.activeName = localStorage.getItem('activeName'))
  },
  methods: {
    handleClick (tab, event) {
      localStorage.setItem('activeName', this.activeName)
      this.$router.push(this.routerData[this.activeName].pathName)
    }
  }
}
</script>
<style lang="less">
.system{
  margin: -50px 0 0 0;
  .wrap{
    .el-tabs__header{
      margin: 0;
      .el-tabs__nav{
        border: none;
        .el-tabs__item{
          background: #ffffff;
          margin-left: 20px;
          border: 1px solid #797979;
          border-bottom: none;
          border-radius: 8px 8px 0px 0px;
        }
      }
    }
  }
  .el-tabs--card>.el-tabs__header .el-tabs__item.is-active {
    background: #cccccc;
    color: #000000;
  }
}
</style>
